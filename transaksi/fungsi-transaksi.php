...
function ambilPeminjaman($kon, $id_pinjam){
	$sql = "SELECT * FROM peminjaman p INNER JOIN anggota a ON peminjaman.id_anggota=anggota.id_anggota INNER JOIN detail_pinjam dp USING(id_pinjam) INNER JOIN buku b ON dp.id_buku=b.id_buku where id_pinjam='$id_pinjam'" ;
	$res = mysqli_query($kon, $sql);
...
function hitungDenda($kon, $id_pinjam, $tgl_kembali){
	...
	$sql = "SELECT tgl_pinjam FROM peminjaman WHERE id_pinjam = $id_pinjam";
	$res = mysqli_query($kon, $sql);

	$data = mysqli_fetch_assoc($res);
	$tgl_jatuh_tempo = $data['tgl_pinjam'];
